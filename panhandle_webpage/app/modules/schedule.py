...
def schedule(orders, cursor, db, max_cap= 50):
    ...
    if len(subsets)!=0:
        ...
        print("final_subset: ",final_subsets, sorted_subsets[final_subsets][1])
        ...
        sorted_final_subsets.append(list(temp_sort_1))
        sorted_final_subsets.append(np.sort(list(temp_sort_2))[::-1])
        sorted_final_subsets = [x for xs in sorted_final_subsets for x in xs]
        for enu, final in enumerate(sorted_final_subsets):
    ...
    #50개에 맞춰 dict_address 첫번때 짜르기
        ...
        print("else", list(dict_address.keys())[0])
        order_idx = 0
        for order in orders:
            if order['address'] == list(dict_address.keys())[0]:
                ...
                if block_num<=max_cap:
                    ...
                    db.commit()
                    print(order)
    cursor.execute("SELECT id, address, red, green, blue, pending FROM orders WHERE pending='3'")
