...
*   2.0 November 2019 - Refactored for IITP program(lattanze).
*
...
*   router.get("/pending",function(req,res)... - returns all pending orders
*   router.get("/pending/:id",function(req,res)... - returns pending orders associated with id
*   router.get("/pendingcustorders/:customer",function(req,res)... - returns pending orders associated with customer
*   router.get("/filled",function(req,res)... - gets all filled orders
*   router.get("/filledcustorders/:customer",function(req,res)... - gets all filled orders associated with customer
*   router.post("/markOrderFilled/:id",function(req,res)... - marks orders as fulfilled
*   router.post("/neworder",function(req,res)... - adds new order to the database
*   router.delete("/deleteOrder/:id",function(req,res)... - deletes order by id
*
...
REST_ROUTER.prototype.handleRoutes= function(router,connection) {
    ...
    router.post("/neworder",function(req,res){
        console.log("Adding order::", req.body.customer,",",req.body.red,",",req.body.blue,",",req.body.green,",",req.body.white,",",req.body.address);
        ...
        var query = "INSERT INTO ??(??,??,??,??,??,??) VALUES (?,?,?,?,?,?)";
        var table = ["orders","customer","red","blue","green","pending","address",req.body.customer,req.body.red,req.body.blue,req.body.green,true,req.body.address];
        query = mysql.format(query,table);
        connection.query(query,function(err,rows){
            if(err) {
                res.json({"Error" : true, "Message" : "Error executing MySQL query"});
